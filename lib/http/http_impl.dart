    ...
    _subscription = _httpParser.listen((incoming) {
      ...
      if (incoming.statusCode == 100) {
        incoming.drain().then((_) {
          ...
          _nextResponseCompleter!.completeError(HttpException(message, uri: _currentUri), stackTrace);
          _nextResponseCompleter = null;
        }, test: (error) => error is HttpException || error is SocketException || error is TlsException);
      } else {
...
class _HttpClient implements HttpClient {
  ...
  bool _isValidToken([String token = "token"]) {
  ...
  Future<_HttpClientRequest> _openUrl(String method, Uri uri) {
    ...
    if (method != "CONNECT") {
      ...
      } else if (_connectionFactory == null && !uri.isScheme("http") && !uri.isScheme("https")) {
...
// [_HttpServer].
abstract interface class ServerSocketBase<T extends Socket> implements Stream<T> {
  int get port;
...
final class _HttpClientBasicCredentials extends _HttpClientCredentials implements HttpClientBasicCredentials {
  String username;
  String password;
...
final class _HttpClientDigestCredentials extends _HttpClientCredentials implements HttpClientDigestCredentials {
  String username;
  String password;
