      ...
      switch (_state) {
        ...
        case _State.RESPONSE_HTTP_VERSION:
          ...
          if (httpVersionIndex < _Const.HTTP1DOT.length) {
            ...
            throw const HttpException("Invalid response line, failed to parse HTTP version");
          }
          break;
        ...
        case _State.RESPONSE_LINE_STATUS_CODE:
          if (byte == _CharCode.SP) {
            ...
            if (byte < 0x30 || byte > 0x39) {
              ...
              throw const HttpException("Invalid response, status code is over 3 digits");
            } else {
  ...
  void _onDone() {
    ...
    if (_incoming != null) {
      if (_state != _State.UPGRADED &&
        ...
        _reportBodyError(const HttpException("Connection closed while receiving data"));
      }
      _closeIncoming(true);
      _controller.close();
      return;
    ...
    if (_state == _State.START) {
      if (!_requestParser) {
        _reportHttpError(const HttpException("Connection closed before full header was received"));
      }
      _controller.close();
      return;
    }
    ...
    if (!_chunked && _transferLength == -1) {
      ...
      // Report the error through the error callback if any. Otherwise
      // throw the error.
      _reportHttpError(const HttpException("Connection closed before full body was received"));
    }
    _controller.close();
  }
