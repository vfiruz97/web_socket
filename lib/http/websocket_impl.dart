    ...
    while ((index < lastIndex) && _state != CLOSED && _state != FAILURE) {
      ...
      if (_state <= LEN_REST) {
        ...
        if (_state == MASK) {
          ...
          _payload.add(Uint8List.view(buffer.buffer, buffer.offsetInBytes + index, payloadLength));
          index += payloadLength;
          if (_isControlFrame()) {
  ...
  void _unmask(int index, int length, Uint8List buffer) {
    ...
    if (length >= blockSize) {
      ...
      if (blockCount > 0) {
        ...
        Int32x4List blockBuffer = Int32x4List.view(buffer.buffer, buffer.offsetInBytes + index, blockCount);
...
typedef /*String|Future<String>*/ _ProtocolSelector = Function(List<String> protocols);

class _WebSocketTransformerImpl extends StreamTransformerBase<HttpRequest, WebSocket> implements WebSocketTransformer {
  final StreamController<WebSocket> _controller = StreamController<WebSocket>(sync: true);
  ...
  static Future<WebSocket> _upgrade(
      HttpRequest request, _ProtocolSelector? protocolSelector, CompressionOptions compression) {
    ...
    if (!_isUpgradeRequest(request)) {
      ...
      response
        ...
        ..close();
      return Future.error(const WebSocketException("Invalid WebSocket upgrade request"));
    }
    ...
    Future<WebSocket> upgrade(String? protocol) {
      ...
      return response
          .detachSocket()
          .then<WebSocket>((socket) => _WebSocketImpl._fromSocket(socket, protocol, compression, true, deflate));
    }
    ...
    if (protocols != null && protocolSelector != null) {
      ...
      return Future<String>(() => protocolSelector(tokenizedProtocols)).then<String>((protocol) {
...
class _WebSocketPerMessageDeflate {
  ...
  RawZLibFilter _ensureDecoder() => decoder ??=
      RawZLibFilter.inflateFilter(windowBits: serverSide ? clientMaxWindowBits : serverMaxWindowBits, raw: true);

  RawZLibFilter _ensureEncoder() => encoder ??=
      RawZLibFilter.deflateFilter(windowBits: serverSide ? serverMaxWindowBits : clientMaxWindowBits, raw: true);
...
class _WebSocketOutgoingTransformer extends StreamTransformerBase<dynamic, List<int>> implements EventSink {
  ...
  _WebSocketOutgoingTransformer(this.webSocket) : _deflateHelper = webSocket._deflate {}
  ...
  void close() {
    ...
    List<int>? data;
    if (code != null) {
      data = [(code >> 8) & 0xFF, code & 0xFF, if (reason != null) ...utf8.encode(reason)];
    }
  ...
  static Iterable<List<int>> createFrame(int opcode, List<int>? data, bool serverSide, bool compressed) {
    ...
    if (mask) {
      ...
      if (data != null) {
        ...
        if (blockCount > 0) {
          ...
          Int32x4 blockMask = Int32x4(mask, mask, mask, mask);
          Int32x4List blockBuffer = Int32x4List.view(list.buffer, list.offsetInBytes, blockCount);
          for (int i = 0; i < blockBuffer.length; i++) {
            blockBuffer[i] ^= blockMask;
          }
        }
    ...
    if (data == null) {
      ...
      Uint8List result = Uint8List(header.length + data.length);
...
class _WebSocketConsumer implements StreamConsumer {
  ...
  StreamController _ensureController() {
    ...
    var stream = controller.stream.transform(_WebSocketOutgoingTransformer(webSocket));
  ...
  Future close() {
    ...
    return _closeCompleter.future.then((_) => socket.close().catchError((_) {}).then((_) => webSocket));
  }
...
class _WebSocketImpl extends Stream with _ServiceObject implements WebSocket {
  ...
  static Future<WebSocket> connect(String url, Iterable<String>? protocols, Map<String, dynamic>? headers,
      {CompressionOptions compression = CompressionOptions.compressionDefault, HttpClient? customClient}) {
    ...
    return (customClient ?? _httpClient).openUrl("GET", uri).then((request) {
      ...
      Future<WebSocket> error(String message) {
        ...
        return Future<WebSocket>.error(WebSocketException(message), callerStackTrace);
      }
      ...
      if (expectedAccept.length != receivedAccept.length) {
        return error("Response header 'Sec-WebSocket-Accept' is the wrong length");
      }
      ...
      return response
          .detachSocket()
          .then<WebSocket>((socket) => _WebSocketImpl._fromSocket(socket, protocol, compression, false, deflate));
    });
  }
  ...
  @override
  StreamSubscription listen(void onData(message)?, {Function? onError, void Function()? onDone, bool? cancelOnError}) {
